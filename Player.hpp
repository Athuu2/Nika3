...
constexpr long highlightSettings = 0xb7a4e90;
constexpr long highlightGenericContext = 0x028d;
constexpr long highlightCurrentContext = 0x028c;
constexpr long highlightVisibilityType = 0x026c; 
constexpr uint32_t HIGHLIGHT_VIS_ALWAYS = 2;
constexpr uint32_t HIGHLIGHT_VIS_LOS = 5;
...
struct Player {
    ...
    int GetPlayerLevel()
    {
        int m_xp = mem::Read<int>(base + OFF_XPLEVEL); //
        if (m_xp < 0) return 0;
    ...
    void setCustomGlow(int health, bool isVisible, bool isSameTeam)
    {
        ...
        mem::Write<unsigned char>(basePointer + OFF_GLOW_ENABLE + contextId, settingIndex);
